...
![Screenshot of the clinical trial simulation agent Shiny app. The left sidebar shows an AI chat interface labeled "do not trust LLM output in here" with a user prompt describing a binary endpoint trial and the AI's response. The main panel displays two tables: "Design assumptions (HUMAN USER MUST REVIEW THIS)" showing simulation parameters for Power and Type I Error scenarios, and "Results (trusted given the assumptions)" showing efficacy probabilities of 0.65 and 0.04 for each scenario.](./images/simulation/app.png)

The human user describes scenarios for a clinical trial design in natural language. 
The LLM translates the human intent into rigorous simulation parameters. 
...
The LLM may hallucinate scenarios, but those scenarios are easy for a human to check.
Given good scenarios, hallucinated results are structurally impossible.

This is where the trusted mini-agent pattern really shines.
The large language model makes the process of conceiving and tuning clinical trial designs more effortless and productive. But because of the structure around the LLM, the question is no longer "do we trust these results?", it's "do these assumptions reflect the scenarios the user wants to simulate?"
...
flowchart LR
  ...
  subgraph Human oversight
    RV --> A["Assumptions table of<br>LLM-generated trial scenarios"]
  end
```
...
At its discretion, the LLM sends [`ellmer`](https://ellmer.tidyverse.org/) the text for a tool call (e.g. `simulate_trial_binary(...)`), at which point [`ellmer`](https://ellmer.tidyverse.org/) runs the tool call instead of simply relaying the text back to [`shinychat`](https://posit-dev.github.io/shinychat/r/).

The simulation tools run the statistical functions with the LLM-supplied assumptions and write both the assumptions and the results to a [`reactiveValues()`](https://shiny.posit.co/r/reference/shiny/0.11/reactivevalues.html) list.
...
Each result that reaches the user originates from a designated, tested tool, never directly from the LLM or the user.

The assumptions table renders the LLM-generated trial scenarios, so the user can check whether the LLM's translation of their query looks correct.
If the simulation tools are correct and the assumptions reflect the design the user wants, then hallucinated results are structurally impossible.
...
These tools don't strictly need oversight because they don't directly produce critical results, they only inform the LLM's reasoning.
...
We simulate many trials by sampling patient responses from the independent normal distributions, then we apply a frequentist hypothesis test to test efficacy in each simulated trial.
...
The `stopifnot()` assertions on the inputs are helpful for catching hallucinated inputs before they even reach human eyes.
...
We simulate the number of responders in each arm from independent binomial distributions, then apply a test of proportions to see if the treatment response rate is significantly higher than that of the control group.
...
Unlike the statistical functions above, these tools accept multiple design scenarios at a time. This is key for evaluating trade-offs in real-world clinical trial simulations.
...
The `ellmer::ContentToolResult()` call at the end of the wrapper function sends the efficacy results back to the LLM.
Accessing the results helps the LLM think and reason, and it is for convenience only.
...
#'   LLM-supplied assumptions and writes the results to a Shiny
#'   `reactiveValues()` list.
...
The Shiny UI has a chat interface, a panel for results, and a panel for the human to review LLM-generated clinical trial simulation assumptions.
...
ui <- bslib::page_sidebar(
  ...
  sidebar = bslib::sidebar(
    title = "AI chat (do not trust LLM output in here)",
    width = "30vw",
