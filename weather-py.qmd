...
Register it once in a `conftest.py` at the project root:
...
Run the tests with `python -m pytest` (rather than bare `pytest`) from the project root.
`app.py` sits at the project root with no package installed, so `python -m pytest` is what puts that root directory on `sys.path`, letting `tests/test_weather_api.py`'s `from app import weather_api` resolve; plain `pytest` does not guarantee this.
Shiny's [end-to-end testing guide](https://shiny.posit.co/py/docs/end-to-end-testing.html) covers additional `conftest.py` options (e.g. pointing at a different app path, or reusing one running app across tests).
The tests require authentication into the AI model, but they can run without a visible browser window in headless mode.

```{.bash}
$ MOCK_WEATHER=true python -m pytest
================= test session starts =================
collected 10 items
